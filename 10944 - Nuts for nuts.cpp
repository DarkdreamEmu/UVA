...
int go(int mask , int pos){
	if(mask == (1<<nuts)-1){
		return distance(location[pos].first,location[pos].second,location[0].first,location[0].second);
	}
	if(dp[mask][pos]!=-1)return dp[mask][pos];
	int ans = 1000000;
	for(int i=1; i<(int)location.size(); i++){
		if(!(mask&(1<<i))){
			ans = min(ans, distance(location[pos].first,location[pos].second,location[i].first,location[i].second) + go(mask|(1<<i) , i));
		}
	}
