...
int go(int pos, int curt, int curm){
	if(curm == m ) return 0;
	if(pos == n) return 0;
	if(dp[pos][curt][curm]!=-1)return dp[pos][curt][curm];
	int ans = 0;
	ans = max(ans,go(pos+1, curt , curm));
	if(ar[pos]+curt <= tt) ans = max(ans, 1 + go(pos+1, ar[pos]+curt , curm));
	ans = max(ans, go(pos, 0 , curm+1));
	return dp[pos][curt][curm]=ans;
}
