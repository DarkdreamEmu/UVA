...
int root(int a[],int i){
    while(a[i]!=i){
        i=a[i];   //chase parent of current element until it reaches root.
    }
    return i;
}
/*modified union function where we connect the elements by changing the root of one of the element */

int UnIon(int a[],int A,int B){
    int root_A=root(a,A);
    int root_B=root(a,B);
    a[root_A]=root_B;            //setting parent of root(A) as root(B).
}

bool find1(int a[],int A,int B){
    if(root(a,A)==root(a,B)){
        return true;              //if A and B have same root,means they are connected.
    }
    else{
        return false;
    }
